<template>
    <div id="terminal" class="terminal">
        ...
        <div class="terminal_input_line terminal">
            <div class="terminal_input_prompt terminal">{{ prompt }}</div><input ref="inputRef" type="text" @keydown.enter="submit" v-model="inputValue" class="terminal_input terminal" autocomplete="off"/>
        </div>
    </div>
    <Network ref="network" @message="handleMessage" @error="handleError" @open="handleOpen" @close="handleClose"/>
</template>
        ...
        methods: {
            ...
            submit() {
                ...
                if (this.promptResolver != null) {
                    ...
                    if (!this.getPasswordState())
                            this.log(`${this.prompt}${inptxt}`);
                    if (inptxt[0] == "/") {
                        // command
                            ...
                            case "auth":
                                this.ask("Username: ").then((username: string) => {
                                    this.ask("Password: ", true).then((password: string) => {
                                        this.$refs.network.sendEvent({event: "AUTHENTICATE", username, password});
                                    });
                                });
                                if (cmd[1] != undefined && cmd[1].trim().length > 0) this.promptResolver(cmd[1].trim());
                                break;
                            case "register":
                                this.ask("Username: ").then((username: string) => {
                        ...
                        this.$refs.network.sendEvent({event: "LINE", data: inptxt});
                    }
                }
            },
            log(...args: any[]) {
